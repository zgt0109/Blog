class Admin::ArticleTypesController < ApplicationController
  ...
  def index
    @article_types = ArticleType.page params[:page]
  end
  ...
  def create
    ...
    respond_to do |format|
      if @article_type.save
        format.html { redirect_to [:admin, @article_type], notice: 'Article type was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @article_type.update(article_type_params)
        format.html { redirect_to [:admin, @article_type], notice: 'Article type was successfully updated.' }
